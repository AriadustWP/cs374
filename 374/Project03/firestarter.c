...
int main(int argc, char ** argv) {
    ...
    iteration = (double *) malloc (n_probs*sizeof(double)); //finish up the work
    ...
    for (i_prob = 0 ; i_prob < n_probs; i_prob++) {
        ...
        for (i_trial=0; i_trial < n_trials; i_trial++) {
            ...
            iteration[i_prob] += burn_until_out(forest_size,forest,prob_spread[i_prob],
                forest_size/2,forest_size/2);
            percent_burned[i_prob]+=get_percent_burned(forest_size,forest);
        }
        percent_burned[i_prob]/=n_trials;
        iteration[i_prob]/=n_trials;

        // print output
        printf("%lf , %lf, %lf\n",prob_spread[i_prob],
            percent_burned[i_prob], iteration[i_prob]);
    }

    // plot graph
    if (do_display==1) {
        xgraphSetup(&thegraph,400,400);
        xgraphDraw(&thegraph,n_probs,0,0,1,1,prob_spread,percent_burned);
        xgraphSetup(&thegraph2,400,400);
        xgraphDraw(&thegraph2,n_probs,0,0,1,40,prob_spread,iteration);
        pause();
    }
