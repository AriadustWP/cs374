 ...
 * vector multiplication. It is the similar to the sample illustrating Chapter 2
 ...
 *
 * Computes the vector square-root operation of A into C. 
 * The 3 vectors have the same number of elements numElements.
 */
__global__
void vectorSqrt(const float *A, float *C, unsigned long numElements)
{
...
int main(int argc, char** argv)
{
    double start;
    double seq_start;
    double copy2DeviceTime;
    double processTime;
    double copy2HostTime;
    double seq_end;
    double end;
    ...
    // 2. Copy the host input vectors A in host memory 
    ...
    // Copy2Device timing //
    ////////////////////////
    copy2DeviceTime = omp_get_wtime() - start;
    ...
    checkErr(err, "Failed to launch vectorSqrt kernel");

    ////////////////////
    // Process timing //
    ////////////////////
    processTime = omp_get_wtime() - (start + copy2DeviceTime);
    ...
    // Copy2Host timing //
    //////////////////////
    copy2HostTime = omp_get_wtime() - (start + copy2DeviceTime + processTime);
...
//    printf("CUDA time: %lf\n", stopTime-startTime); 
    ...
    seq_end = omp_get_wtime() - seq_start;
...
//    printf("Normal time: %lf\n", stopTime-startTime); 
    ...
    printf("Done\n");
    printf("\nSequential Computation Time: %f\n", seq_end);
    printf("\nCUDA Computation Time\nHost2Device copy time: %f\nProcessing time: %f\nDevice2Host copy time: %f\nTotal time: %f\n", copy2DeviceTime, processTime, copy2HostTime, end);
    return 0;
}
